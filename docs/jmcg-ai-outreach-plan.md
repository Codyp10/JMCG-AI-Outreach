...
overview: "JMCG AI Outreach implementation blueprint (v2): SAM=16,000 / 90 days, 18+2 backup mailboxes, Jordan Platten / Affluent.co–style architecture, cost-gated waterfall + phone enrichment only for score ≥50, tag-based Leverage Library matching, configurable 4–5 touch sequences, monthly self-healing optimization, two-tier QA (pass/regenerate), multi-turn reply agent with human escalation, and BLOCKING compliance before SMS/voice. Build in Cursor (IDE); use Claude as the coding agent and primary model layer for AI steps."
todos:
  - id: supabase-schema
    content: "Model Supabase tables with v2 additions: leads (with cycle_number, consent fields, phone_enriched), enrichment_runs (with cumulative_cost), scores, sequences, messages, qa_results (simplified pass/regenerate/failed_qa), experiments, replies (with escalation_reason), cooldown_queue, channel_dispatch, optimization_log, mailbox_health"
    status: pending
  ...
  - id: orchestration-workers
    content: "Implement webhook + cron workers: waterfall enrichment (with cost cap), phone enrichment (score ≥50 only), scoring, copy generation, QA gate (pass/regenerate, max 3 attempts), send, multi-turn reply agent (with human escalation triggers, <2min response SLA), monthly self-optimization job"
    status: pending
  - id: leverage-library
...
# JMCG AI Outreach — Implementation Blueprint (v2)

**Assumptions (v2):** Enriched **SAM = 16,000** verified contacts. Architecture aligned with **Jordan Platten / Affluent.co**–style automation. Niche/offer/ICP: **Johnson Marketing & Consulting (JMCG)** service verticals—populate case studies before launch. **Smartlead** for sending; **Supabase** for data; **Cursor** as the IDE for building and maintaining the system; **Claude** as the coding agent and model layer for AI orchestration (workers, prompts, copy/QA/reply/analysis jobs).
...
- **Supabase:** See **Section 2** (schema) and **Section 7** for tables and integrations.
...
## 2. Data & Scoring Blueprint (0–100)

### Purpose — Surround Sound (with compliance prerequisites)

- **All leads (any score):** Full **email** sequence (see **Section 3** for configurable 4–5 touches).
...
## 3. Copywriting Logic & Prompt Engineering
...
- **Monthly optimization** (Section 4) decides 4 vs 5: if Bump 3 adds meaningful incremental replies, keep 5; if negligible, revert to 4.
...
## 4. Quality Assurance & Monthly Self-Healing Optimization
...
## 5. Agent Workflows — AI Appointment Setter (multi-turn)
...
- **Objection persistence:** Up to **5** back-and-forth exchanges on an objection; if no booking, mark **`objection_exhausted`** and enter **cooldown** (Section 6).
...
- **Supabase:** Source of truth: leads (with consent + cycle fields), scores, sequences, messages, **`qa_results`**, **`experiments`**, **`replies`**, **`optimization_log`**, **`mailbox_health`**, **`cooldown_queue`**, **`channel_dispatch`**.
- **Smartlead:** Send, webhooks (replies/opens as available), per-mailbox limits, backup rotation.
- **Cursor + Claude:** Use **Cursor** as the primary IDE. Use **Claude** (coding agent in Cursor, plus **Claude API** or equivalent at runtime where needed) to implement and operate orchestration: waterfall (cost cap), scoring, phone enrichment (≥50), copy generation, QA (pass/regenerate), send orchestration, **multi-turn** reply agent, **monthly** optimization job, Slack/email summaries.
- **Integrations:** SMS/WhatsApp/voicemail/direct mail behind **`channel_dispatch`**; **BLOCKING** compliance before SMS/voice go-live.
...
Lock **JMCG** case studies into the Leverage Library, finalize **ICP weights** within the 0–100 rubric, complete **legal/compliance** and consent capture, and wire **Slack** + override workflow for monthly optimization reports.
